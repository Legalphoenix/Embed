...
# Texts to update and their respective IDs with proper indentation
texts_to_update = {
    "dd890f03-2a7a-4d39-9e0d-313fedbc31ab": """Article 58
Powers
...
6. Each Member State may provide by law that its supervisory authority shall have additional powers to those referred to in paragraphs 1, 2 and 3. The exercise of those powers shall not impair the effective operation of Chapter VII.""",
    "d3fd03f5-01a2-486f-ba14-7ef8bda45132": """Article 37
...
# Process each text and update the full_preview_text in the collection
for doc_id, text in texts_to_update.items():
    print(f"Updating full_preview_text for document ID: {doc_id}")
    update_full_preview_text(collection, doc_id, text)
...
# IDs to delete
ids_to_delete = [
    "1ca4bb01-c211-470c-8c97-c20f835ba388",
    "1ec476db-8606-41e9-8880-585acedb321c",
    "97765acf-3d6a-48a3-a82f-0f9ec233851f",
    "df0d3acb-300b-4221-863d-3eb5713d11ff",
    "e5e90a25-ab4d-47f0-b18b-858025b3dcbe"
]
...
# Function to find and print items where 'full_preview_text' begins with specific text
def find_and_print_by_full_preview_text_prefix(collection, prefix):
    results = collection.get(include=["metadatas"])
    metadatas = results['metadatas']
    ids = results['ids']

    for doc_id, metadata in zip(ids, metadatas):
        full_preview_text = metadata.get("full_preview_text", "")
        if full_preview_text.startswith(prefix):
            print(f"ID: {doc_id}")
            print(f"Full Preview Text: {full_preview_text}\n")

# Search and print items where 'full_preview_text' begins with 'Article 58'
find_and_print_by_full_preview_text_prefix(collection, "Article 58")

# Search and print items where 'full_preview_text' begins with 'Article 37'
find_and_print_by_full_preview_text_prefix(collection, "Article 37") '''
...
# Function to check and print items without 'full_preview_text' metadata
def check_and_print_missing_metadata(collection):
    ...
    for doc_id, metadata, document in zip(ids, metadatas, documents):
        if "full_preview_text" not in metadata:
            print(f"ID: {doc_id}")
            print(f"Chunk Text: {metadata.get('chunk_text', 'No chunk_text available')}")

# Execute the function
check_and_print_missing_metadata(collection)'''
